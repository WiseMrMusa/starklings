...
impl StateImpl of StateTrait {
    ...
    // Remember: When passing a tuple as a function argument, you'll need extra parentheses: fn function((t, u, p, l, e))
        let State{color, position, quit, } = self;
        self = match message {
            Message::Quit(()) => State { color: color, position: position, quit: true,  },
            Message::Echo(msg) => {
                msg.print();
                State { color: color, position: position, quit: quit,  }
                },
            Message::Move(Point { x, y}) => State { color: color, position: Point {x,y}, quit: quit,  },
            Message::ChangeColor((
                red, green, blue
            )) => State { color: (red, green, blue), position: position, quit: quit,  },
        }
    }
}
